...
var valids = []string{
	...
	`package p; func f() { selectp { case <- c: case c <- d: case c <- <- d: case <-c <- d: } };`,
	`package p; func f() { selectp { case x := (<-c): } };`,
...
var invalids = []string{
	...
	`package p; func f() { selectp { case _ <- chan /* ERROR "expected expression" */ int: } };`,
